...
public class LuceneServicesController {
	...
	@RequestMapping(value="/searchHighlighted", method=RequestMethod.GET)
	@ResponseStatus(value=HttpStatus.OK)
	public String queryLuceneHighlighted(@RequestParam(value="query") String query,
							  @RequestParam(value="count", required = false) String countStr)
			throws LuceneSearcherException, InvalidLuceneQueryException {
		if (!query.trim().isEmpty()) {
			int count = QUERY_DEFAULT_RECORDS;
			...
			String results = indexSearcher.highLight(query, count, showAvailable);
