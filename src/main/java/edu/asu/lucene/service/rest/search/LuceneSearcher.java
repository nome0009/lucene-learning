...
import edu.asu.lucene.service.rest.highlighter.TextFormatter;
...
public class LuceneSearcher {
	...
	public LuceneSearcher(@Value("${lucene.index.location}") String indexLocation) throws LuceneSearcherException {
		try {
			...
			queryParser = new QueryParser("body", analyzer);
...
//			reader.close();
	...
	public Result searchIndex(String querystring, int numRecords, boolean showAvailable) throws LuceneSearcherException, InvalidLuceneQueryException {
		...
		try {
			...
			Term termInstance = new Term("body", querystring);
			long totalTermFreq = reader.totalTermFreq(termInstance);
			logger.info(String.format("Total hits for term '%s': %d", querystring, totalTermFreq));

			indexSearcher = new IndexSearcher(reader);
			query = queryParser.parse(querystring);
			logger.info(String.format("'%s' ==> '%s'", querystring, query.toString()));
			if(showAvailable){
	...
	public String highLight(String querystring, int numRecords, boolean showAvailable) throws LuceneSearcherException, InvalidLuceneQueryException {
		CharArraySet stopWordsOverride = new CharArraySet(Collections.emptySet(), true);
		Analyzer analyzer = new StandardAnalyzer(stopWordsOverride);
		IndexReader reader = null;
		IndexSearcher indexSearcher = null;
		...
		try {
			...
			documents = indexSearcher.search(query, numRecords);
			TextFormatter htmlFormatter = new TextFormatter();
			Highlighter highlighter = new Highlighter(htmlFormatter, new QueryScorer(query));

			for (int i = 0; i < reader.maxDoc(); i++) {
				int id = documents.scoreDocs[i].doc;
				Document doc = indexSearcher.doc(id);
				String text = doc.get("file-name");
				TokenStream tokenStream = TokenSources.getAnyTokenStream(indexSearcher.getIndexReader(), id, "file-name", analyzer);
				...
				for (TextFragment aFrag : frag) {
					if ((aFrag != null) && (aFrag.getScore() > 0)) {
						highlightedText.append(aFrag.toString());
					}
				}
				//Term vector
				highlightedText.append("\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
				text = doc.get("body");
				tokenStream = TokenSources.getAnyTokenStream(indexSearcher.getIndexReader(), documents.scoreDocs[i].doc, "body", analyzer);
